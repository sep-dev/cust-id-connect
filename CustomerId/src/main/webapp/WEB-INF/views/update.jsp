		...
		<table>
			...
			<tr>
				...
				<td><input type="name" placeholder="ここに氏名を入力" size="50" required="required"
					pattern="^[-a-zA-Z0-9ぁ-ゑんァ-ヶ一-龠々][-a-zA-Z0-9ぁ-ゑんァ-ヶ一-龠々 　]{0,254}$"
			...
			<tr>
				...
				<td><input type="address" placeholder="ここに住所を入力" size="50" required="required"
					pattern="^[-a-zA-Z0-9ぁ-ゑんァ-ヶ一-龠々][-a-zA-Z0-9ぁ-ゑんァ-ヶ一-龠々 　]{0,254}$"
					title="住所は半角英数字、かな漢字のみで記入してください。"/></td></td><td><font color="red">＜"必須"＞</font></td>
			</tr>
			<tr>
				<td>電話番号</td>
				<td><input type="tel" placeholder="ここに電話番号を入力 " size="50" required="required" pattern="\d{1,5}-\d{1,4}-\d{3,5}"
					title="電話番号は、市外局番からハイフン（-）を入れて記入してください。"/></td></td><td><font color="red">＜"必須"＞</font></td>
			</tr>
			...
			<tr>
				<td></td><td><input type="submit" value="更新" onclick="kakunin(this.form);return false" /> <input type="reset" />
